...
export default function ProductCard({ obj }) {
	...
	return (
		...
		>
			...
			<div
				className={` absolute ${obj.packS ? 'top-10 ..5x2:top-8' : 'top-1.5'
					} left-1.5 z-10`}
			>
			...
			{obj.prodDayLabel && (
				<>
					...
					<div
						className={`${obj.packS ? ' top-70px' : ' top-11'
							} absolute left-1.5 z-10 hidden ..5x2:flex items-center justify-center border border-label-green bg-white rounded-56px w-9 h-9`}
					>
			...
			{obj.packLabel && (
				<>
					<div
						className={`${obj.prodDayLabel ? 'left-14' : 'left-1.5'
							} absolute top-16 z-10 w-60px h-60px flex items-center justify-center rounded-56px ..5x2:hidden`}
					>
						<div>
							<svg
								className=""
					...
					<div
						className={`${obj.prodDayLabel ? 'top-72px' : 'top-11'
							} hidden ..5x2:flex absolute left-1.5  z-10 flex-col items-center justify-center rounded-56px w-9 h-9`}
					>
						<svg
							className="absolute"
							width="36"
