...
const SetLocation = ({ showL, setShowL, setCurrentCity, currentCity }) => {
	const [show, setShow] = useState(false),
		...
		[currentCountry, setCurrentCountry] = useState({ name: 'Российская Федерация', src: cont })
					...
					<section className={`px-8 bg-white py-6 bottom-0 w-460px h-full left-0 top-0 `}>
						<div className="relative h-full">
							...
							<div className="relative w-full mt-3 text-left">
								...
								<div className={`absolute left-0 z-10 -mt-1 w-full bg-white shadow-lg border-2   border-gray-smart ${show ? "block" : "hiddenCustom"}`} role="menu"  >
									<div className="relative">
										...
										<input type="text" onChange={setValueCountry} value={countryValue} placeholder="Введите страну" className="border-b-2 text-sm bg-white-BG_FILTER pl-10 outline-none py-3 w-full border-gray-smart" />
										{countryValue.length > 0 && <svg onClick={clearValueCountry} className="absolute right-2 cursor-pointer top-3" width="23" height="23" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
									...
									<div className='max-h-80 overflow-auto'>
										{countries.map((country, i) => <SetLocationItem src={country.src} title={country.name} key={i} setCurrentCountry={setCurrentCountry} setShow={setShow} />)}
							...
							<div className="relative mt-2">
								...
								<input type="text" value={cityValue} onChange={setValue} placeholder="Поиск" className="w-full pl-10 outline-none border-gray-smart border-2 bg-white px-4 py-3 text-sm font-medium " />
								{cityValue.length > 0 && <svg onClick={clearValue} className="absolute right-2 cursor-pointer top-3" width="23" height="23" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
									<path d="M13.3337 6.6665L6.66699 13.3332M6.66699 6.6665L13.3337 13.3332" stroke="#949792" strokeWidth="1.5" strokeLinecap="round" />
								</svg>}
								{cityValue.length > 0 && !currentCity && <section className="absolute confirm -bottom-18 p-2 w-full   left-0 bg-white">
									<p className="font-medium mb-4">Результат поиска</p>
									{findCities.map((city, i) => <div onClick={() => changeCurrentCity(city.name)} key={i} className="pb-4 hover:bg-gray-light2 transition-colors cursor-pointer">
										<p className="font-medium">{city.name}</p>
										<p className="text-gray-quick-silver text-sm">{city.subName}</p>
									</div>)}
							...
							<div className="flex flex-wrap mt-8">
								{citiesRecomend.map((city, i) => <p key={i} onClick={() => changeCurrentCity(city.name)} className="text-primary-active text-xs px-4 mr-3 mb-3 cursor-pointer py-2 bg-gray-light rounded-xl">{city.name}</p>)}
							</div>
