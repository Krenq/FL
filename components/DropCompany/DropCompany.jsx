...
const DropCompany = ({ show, setShow, showDrop, scrollMenu }) => {
	return (
		<>
			<div onMouseEnter={showDrop} onMouseLeave={setShow} className={`${show.showCompany ? 'active' : "hiddenCustom"} fixed top-70 ${scrollMenu ? 'top-10' : ""} left-0 right-0 bg-red h-10 z-30`}></div>

			<section onMouseLeave={setShow} onMouseEnter={showDrop} className={`absolute .2x1:pt-2  flex z-20 right-0 left-0 h-56 .3x1:pt-0 .3x1:h-44 .3x1:items-start .3x1:pt-5  transition-all bg-white ${scrollMenu ? 'mt-6' : ""} mt-9 ${show.showCompany ? 'active' : "hiddenCustom"}`}>
