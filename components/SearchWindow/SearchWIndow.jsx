...
const SearchWindow = ({ scrollMenu, setIsHover, isHoverBasket, setCloseHover }) => {
	...
	return (
		<>
			...
			<section onMouseEnter={setIsHover} onMouseLeave={setCloseHover} className={`absolute ${isHoverBasket ? '!flex' : "!hidden"} dropDownShadow border border-gray right-91.7% -mr-1.5 top-16 bg-white searchwindow ${scrollMenu ? 'top-14' : ''}`}>
				<div className="w-475px .4x2:w-380px">
					...
					<div className='flex justify-between  border-b border-gray   mt-5'>
						<p onClick={setGifs} className={`ml-9 text-13px text-black-70pe .2x1:ml-2 .2x1:text-10px font-medium transition-all cursor-pointer hover:text-black hover:border-b hover:pb-0 pb-1 font-montserrat ${nowFilter.gifts ? 'activeSpecialDrop' : ''}`}>Акции и подарки</p>
						<p onClick={setIdea} className={`text-13px text-black-70pe font-medium .2x1:text-10px transition-all cursor-pointer hover:text-black hover:border-b hover:pb-0 pb-1 font-montserrat ${nowFilter.idea ? 'activeSpecialDrop' : ''}`}>Идеи подарков</p>
						<p onClick={setRecomend} className={`mr-9 text-13px text-black-70pe .2x1:mr-2 .2x1:text-10px font-medium transition-all cursor-pointer hover:text-black hover:border-b hover:pb-0 pb-1 font-montserrat ${nowFilter.recomend ? 'activeSpecialDrop' : ''}`}>Рекомендации</p>
					</div>
				...
				<div className='productNum .2x1:mx-2  mt-2 mx-4'>
					...
					<div className='mt-4 bg-white-BG_FILTER .2x1:px-2 p-4 pt-2 pb-3'>
